...
public class ArticleController {
    ...
    @GetMapping("/articles")
    public List<GetArticlesDto.Response> getArticles(@RequestParam(required = false) String searchTag, @PageableDefault(size=5,sort = "idx", direction = Sort.Direction.DESC) Pageable pageable){
        List<Article> articles = articleService.getArticles(searchTag, pageable);
